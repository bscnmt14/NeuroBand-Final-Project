...
def run_model(features_df):
    print("\n[PLUGIN] Executing Early Fusion KNN...")
    ...
    knn_df = pivot_df.reset_index().dropna()
    feature_cols = [col for col in knn_df.columns if col.startswith('sensor_')]
    ...
    for gesture in knn_df['gesture_label'].unique():
        gesture_trials = sorted(knn_df[knn_df['gesture_label'] == gesture]['trial_index'].unique())
        
        split_idx = int(len(gesture_trials) * hp.TRAIN_SPLIT_RATIO)
        if split_idx == len(gesture_trials): split_idx = max(1, len(gesture_trials) - 1)
        elif split_idx == 0 and len(gesture_trials) > 0: split_idx = 1
            
        train_trials.extend(gesture_trials[:split_idx])
        val_trials.extend(gesture_trials[split_idx:])
        
    train_mask = knn_df['trial_index'].isin(train_trials)
    val_mask = knn_df['trial_index'].isin(val_trials)
    
    X_train = knn_df.loc[train_mask, feature_cols]
    y_train = knn_df.loc[train_mask, 'gesture_label']
    X_val = knn_df.loc[val_mask, feature_cols]
    y_val = knn_df.loc[val_mask, 'gesture_label']
    ...
    print(f" -> Memorizing {len(X_train)} windows, Querying {len(X_val)} windows...")
    ...
    # ==========================================
    final_predictions = []
    CONFIDENCE_THRESHOLD = hp.CONFIDENCE_THRESHOLDS['knn']
    ...
    print("\nEarly Fusion KNN Confusion Matrix (Percentages %):")
    ...
    disp.plot(cmap='Purples', values_format='.1f')
    plt.title(f"Early Fusion KNN\nK=5 | Weights='Distance' | Cutoff: {CONFIDENCE_THRESHOLD*100}%")
    plt.show()
    
    return {'early_fusion_knn': clf}, {'early_fusion_knn': scaler}, val_precision