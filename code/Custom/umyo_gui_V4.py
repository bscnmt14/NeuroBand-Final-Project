...
class FilterPanel(QtWidgets.QGroupBox):
    ...
    def _build_ui(self):
        ...
        self.bs_label = QtWidgets.QLabel(r"Band-Stop Deltas (Left &#916;, Right &#916; in Hz):")
...
class DevicePanel(QtWidgets.QWidget):
    ...
    def __init__(self, index: int, emg_len=2000, acc_len=200, parent=None):
        ...
        self.spg_img_buffer = np.zeros((self.sp_len, 4), dtype=float)

        # Log Scale normalization tracker
        self.global_log_max = 8.0

        self.is_cold_start = True

        # --- FIX: Start Time for 5-second delay ---
    ...
    def _build_ui(self):
        ...
        # Spectrum - 4-Band Spectrogram
        self.spg_plot = pg.PlotWidget(title="Spectrogram (0-50, 50-150, 150-250, >250 Hz)")
        self.spg_plot.showGrid(x=False, y=False)
        self.spg_plot.setMouseEnabled(x=False, y=False)
        self.spg_plot.setMenuEnabled(False)

        self.spg_img = pg.ImageItem()
        self.spg_plot.addItem(self.spg_img)

        # Custom Colormap: Black -> Blue -> Green -> Red
        ...
        self.spg_img.setLookupTable(colormap.getLookupTable(256))
        self.spg_img.setLevels([0, 1])
    ...
    def update_from_device(self, dev):
        ...
        if hasattr(dev, "data_array") and hasattr(dev, "data_count"):
            ...
            if len(samples) > 0:
                if self.is_cold_start:
                    self.filtered_emg_buffer[:] = samples[0]
                    self.emg_buffer[:] = samples[0]
        ...
        # --- FIX: Check if 5 seconds have passed ---
        if time.time() - self.startup_time > 5.0:
            try:
                current_vals = np.zeros(4, dtype=float)

                if len(self.filtered_emg_buffer) > 256:
                    # detrend='linear' to remove DC slope artifacts
                    f, Pxx = signal.welch(self.filtered_emg_buffer, fs=self.FS, nperseg=256, detrend='linear',
                                          scaling='spectrum')

                    bands = [(0, 50), (50, 150), (150, 250), (250, self.FS / 2)]

                    for i, (low, high) in enumerate(bands):
                        indices = np.where((f >= low) & (f < high))
                        if len(indices[0]) > 0:
                            current_vals[i] = np.sum(Pxx[indices])

                    log_vals = np.log10(current_vals + 1.0)

                    current_max_log = np.max(log_vals)
                    if current_max_log > self.global_log_max:
                        self.global_log_max = current_max_log
                    else:
                        self.global_log_max *= 0.999

                    dynamic_range = 4.0
                    floor = max(0, self.global_log_max - dynamic_range)

                    normalized_vals = (log_vals - floor) / (self.global_log_max - floor)
                    normalized_vals = np.clip(normalized_vals, 0, 1)

                self.spg_img_buffer = np.roll(self.spg_img_buffer, -1, axis=0)
                self.spg_img_buffer[-1] = normalized_vals
                self.spg_img.setImage(self.spg_img_buffer, autoLevels=False)

            except Exception as e:
                pass

        # Update Accel / Info (Always runs)
...
class UmyoGui(QtWidgets.QMainWindow):
    ...
    def start_session(self):
        ...
        self.current_label = "beginning"  # Fix from previous step
        self.full_session_data = []
        self.session_start_time = 0.0
    ...
    def _save_file(self):
        ...
        param_clean = re.sub(r'[\\/*?:"<>|]', "", param_raw)
        param_clean = param_clean.replace(" ", "_")
